...
def generate_full_inventory():
    ...
    sorted_items = sorted(manufacturer_data.values(), key=lambda item: item.manufacturer)
...
def generate_past_service_date_inventory():
    ...
    sorted_items = sorted(past_service_items, key=lambda item: convert_to_datetime(item.service_date))
...
def generate_damaged_inventory():
    ...
    sorted_items = sorted(damaged_items, key=lambda item: item.price, reverse=True)
